...
function headOne(eim, player) {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			var pIter = eim.getPlayers().iterator();
			while (pIter.hasNext()) {
				pIter.next().getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"The Enormous Creature is Approaching from the Deep Cave..."));
			}
		}
...
function headTwo(eim, player) {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			var pIter = eim.getPlayers().iterator();
			while (pIter.hasNext()) {
				pIter.next().getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"Watch out for Horntail... Be prepared for a long fight."));
			}
		}
