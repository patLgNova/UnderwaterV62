importPackage(Packages.Packages.tools);
importPackage(Packages.Packages.server.life);
		...
		if (stage5done == null) {
			if (playerStatus) { 
				...
				var passes = cm.haveItem(4001008,10);
                                var randomnx = Math.floor(Math.random() * nx.length);
				if (passes) {
...
function rectanglestages (cm) {
	...
	if (playerStatus) { 
		if (status == 0) {
			...
			if (preamble == null) {
				cm.sendNext("Hello! Welcome to " + nthtext + " stage. You will see some barrels nearby. 3 of these barrels are connected to the portal leading to the next stage.#b3 group members need to find the correct barrels and stay on top of them #k to complete the stage. BUT, for the answer count, you must be securely held in the center of the barrel, not at the border. And only 3 members of your group can stay on top of the barrels. When the members are on the leader of the group should #bclick twice on me to see if the answer is correct or not #k. Now, find the correct barrels! ");
				...
				if (complete != null) {	
					...
					cm.sendNext("Please hurry onto the next phase, the portal is open!");
				} else { 
					var totplayers = 0;
			...
			if (testcombo || debug) {
				...
				if (debug) {
					...
					cm.sendNext("It seems that you still have not found the 3 barrels. Think of a different combination of the barrels. And do not forget that only 3 members can stay on top of the barrels, firm in the center so that the response count as correct. Continue!");
                                    cm.dispose();
					}
				}
			}
		...
		if (status == 0) {
			var complete = eim.getProperty(curMap.toString() + "stageclear");
			if (complete != null) {
				cm.sendNext("Please hurry on to the next phase, the portal is open!");
			} else {
				cm.sendNext("Please, ask the leader of the group to talk to me.");
				cm.dispose();
			}
		} else {
			var complete = eim.getProperty(curMap.toString() + "stageclear");
