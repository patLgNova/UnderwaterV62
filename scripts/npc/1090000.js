...
function action(mode, type, selection) {
	...
	cm.sendOk("Hey I'm disabled for Open Beta!");
	cm.dispose();
	}
}
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
                ...
                if (cm.getLevel() >= 30 
                    && cm.getJob().equals(Packages.client.MapleJob.PIRATE)) {
                        if (cm.getQuestStatus(2192).equals(Packages.client.MapleQuestStatus.Status.COMPLETED)) {
                            status = 20;
                     ...
                     else if (cm.getQuestStatus(2191).equals(Packages.client.MapleQuestStatus.Status.COMPLETED)) {
                            status = 30;
                     ...
                     else if (cm.getQuestStatus(2191).equals(Packages.client.MapleQuestStatus.Status.STARTED)) {
                        status = 15;
           cm.sendYesNo("Would you like to take the test for the second job advancement?");
                    } else if (cm.getQuestStatus(2192).equals(Packages.client.MapleQuestStatus.Status.STARTED)) {
                        status = 10;
           cm.sendYesNo("Would you like to take the test for the second job advancement?");
                    } else {
               ...
               } else if (cm.getQuestStatus(32100).equals(MapleQuestStatus.Status.STARTED) && cm.getJob().equals(MapleJob.BRAWLER) || cm.getJob().equals(MapleJob.GUNSLINGER)) {
            ...
            cm.sendNext("From here on out, you are a #bGunslinger#k. Gunslingers are notable for their long-range attacks with sniper-like accuracy and of course, using Guns as their primary weapon. You should continue training to truly master your skills. If you are having trouble training, I'll be there to help.");
                cm.changeJob(Packages.client.MapleJob.GUNSLINGER);
                cm.getChar().gainAp(5);
               // cm.getChar().gainSp(1);
            cm.removeAll(4031857);
        } else if (status == 22) {
        ...
        cm.sendNext("From here on out, you are a #bBrawler#k. Brawlers rule the world with the power of their bare firsts... Which means they need to train the body more than others. If you have any trouble training, I'll be more than happy to help.");
                cm.changeJob(Packages.client.MapleJob.BRAWLER);
                cm.getChar().gainAp(5);
                //cm.getChar().gainSp(1);
            cm.removeAll(4031856);
        } else if (status == 32) {
