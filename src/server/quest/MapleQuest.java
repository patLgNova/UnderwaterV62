...
public class MapleQuest {
    ...
    public boolean canComplete(MapleCharacter c, Integer npcid) {
        ...
        if (c.getQuest(MapleQuest.getInstance(3239)).getStatus().equals(Status.STARTED)) {
            return true;
        }
        return true;
    }
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
		if (autoPreComplete || canComplete(c, npc)) {
		   for (MapleQuestAction a : completeActs.values()) {
		    ...
		    forceComplete(c, npc);
                    c.getClient().getSession().write(MaplePacketCreator.showQuestComplete()); // Quest completion
		    for (MapleQuestAction a : completeActs.values()) {
		        a.run(c, selection);
		    }
		}
		...
		try {
			for(MapleData quest : questInfo.getChildren()) {
				...
				quests.put(questID, new MapleQuest(questID));
                    ...
                    //System.out.println("Quest not loaded is: " + questInfo + "\n"+questReq + "\n" + questAct + "\n");
			//ex.printStackTrace();
		}
            }
}
