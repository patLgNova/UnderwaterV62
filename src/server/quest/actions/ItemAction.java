...
public final class ItemAction extends MapleQuestAction {
    ...
    public void processData(MapleData data) {
        for (MapleData iEntry: data.getChildren()) {
            ...
            int count = MapleDataTool.getInt(iEntry.getChildByPath("count"));
        ...
        for (ItemData iEntry: items.values()) {
                ...
                if (iEntry.getProp() == -1) {
                    if (extSelection != extNum++) {
                        if (chr.isGM()) {
                            ...
                            extSelection = 1
                            extNum = 1
                            Selected blue potions
                            extSelection = 0
                            extNum = 2
                            */
                            chr.dropMessage("extSelection" + extSelection);
                            chr.dropMessage("extNum" + extNum);
                        }
                        continue;
                    }
            ...
            if (iEntry.getCount() <= 0) { // Remove Items
                ...
                if (type.equals(MapleInventoryType.EQUIP)) {
                    if (quantity < 1) {
                        quantity = 1; //Quantity is 1 when we can't invert. It's an equip, so.
                    }
                ...
                if (quantity < 1) {
                    quantity = chr.getItemQuantity(iEntry.getId(), false); //Take everything when we can't invert. 
                }
                MapleInventoryManipulator.removeById(chr.getClient(), type, iEntry.getId(), quantity, true, false);
                chr.getClient().getSession().write(MaplePacketCreator.getShowItemGain(iEntry.getId(), (short) iEntry.getCount(), true));
            } else {
    ...
    public boolean check(MapleCharacter chr, Integer extSelection) {
        ...
        for (ItemData item: items.values()) {
            ...
            if (item.getCount() > 0) {
                ...
                IItem toItem = new Item(item.getId(), (byte) 0, (short) item.getCount());
        ...
        for (Integer itemID: props.values()) {
            ...
            IItem toItem = new Item(itemID, (byte) 0, (short) 1);
            itemList.add(new Pair < > (toItem, type));
        }

        if (!MapleInventory.checkSpots(chr, itemList)) {
            chr.dropMessage(1, "Please make sure you have enough room in your inventory for the item.");
            return false;
        }
        return true;
    }
    ...
    //            }
    //            if (item.getJob() < 100) {
    //                    if (MapleJob.getBy5ByteEncoding(item.getJob()).getId() / 100 != chr.getJob().getId() / 100) {
    //                    return false;
    //                }
    //           } else {
    ...
    private boolean canGetItem(ItemData item, MapleCharacter chr) {
        ...
        if (item.getJob() != -1) {
            //			if (item.getJob() != chr.getJob().getId()) {
            //                            chr.dropMessage("canGetItem registered. Item ID is " + item.getId());
            //                            chr.dropMessage("canGetItem registered2. Item job is " + item.getJob());
            //			  return false;
            //			} else 
            if (GameConstants.JobToEncoding(chr.getJob().getId()) != item.getJob()) {
                return false;
            } else if (item.getId() == 1092003) {
                return true;
            }
            //                        if (MapleJob.getBy5ByteEncoding(item.getJob()).getId() / 100 != chr.getJob().getId() / 100) {
            //                            return false;
